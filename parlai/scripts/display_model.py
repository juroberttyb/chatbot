...
@register_script('robert_display_model', aliases=['dm'])
class RobertDisplayModel(DisplayModel):
    def simple_display(self, opt, world, turn):
        if opt['batchsize'] > 1:
            raise RuntimeError('Simple view only support batchsize=1')
        teacher, response = world.get_acts()
        if turn == 0:
    ...
    def run(self):
        ret = self.display_model(self.opt)
        return ret
